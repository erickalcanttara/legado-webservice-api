...
public class VerificarDisponibilidadeBloqueioTempTest {
    ...
    @ParameterizedTest
    @CsvFileSource(resources = "/massaDeTestes/VerificarDisponibilidadeBloqueioTemp/VerificarDisponibilidadeBloqueioTempMassaDeTestes.csv", numLinesToSkip = 1, delimiter = ';')
    public void VerificarDisponibilidadeBloqueioTemp_SC_OK(String ReferenceTest, String CPF, String id_conta, String id_cartao, String statusCartao, String estagioCartao) throws IOException, ParserConfigurationException, SAXException {
        ...
        String requestBody = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:br=\"br.com.conductor.RealizeWs.CartaoService\" xmlns:br1=\"br.com.conductor.RealizeWs.Cartao.Contracts\">\n" +
                "   <soapenv:Header/>\n" +
                "   <soapenv:Body>\n" +
                "      <br:VerificarDisponibilidadeBloqueioTemp>\n" +
                "         <!--Optional:-->\n" +
                "         <br:request>\n" +
                "            <!--Optional:-->\n" +
                "            <br1:CPF>"+ CPF + "</br1:CPF>\n" +
        ...
        Response proxyResponse =
                given().
                        ...
                        header("SOAPAction", "\"br.com.conductor.RealizeWs.CartaoService/CartaoService/VerificarDisponibilidadeBloqueioTemp\"").
                        accept("*/*").
                        body(requestBody).
                        log().method().
                        log().uri().
        ...
        File testDirList = new File("src/test/resources/" + "/verificarDisponibilidadeBloqueioTemp/");
        if (!testDirList.exists()){
            testDirList.mkdirs();
        }

        File testDir = new File("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/");
        if (!testDir.exists()){
            testDir.mkdirs();
        }

        FileWriter file = new FileWriter("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/" + ReferenceTest + "-in.xml");
        file.write(proxyResponse.prettyPrint());
        file.flush();
        file.close();

        FileWriter fileRq = new FileWriter("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/" + ReferenceTest +  "-rq.xml");
        fileRq.write(requestBody);
        fileRq.flush();
        fileRq.close();

        Path pathFileProxy = Paths.get("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/" + ReferenceTest +  "-in.xml");
        ...
        FileWriter file2 = new FileWriter("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/" + ReferenceTest +  "-ws.xml");
        file2.write(legadoResponse.prettyPrint());
        file2.flush();
        file2.close();

        Path pathFileLegado = Paths.get("src/test/resources/verificarDisponibilidadeBloqueioTemp/" + ReferenceTest + "/" + ReferenceTest +  "-ws.xml");
